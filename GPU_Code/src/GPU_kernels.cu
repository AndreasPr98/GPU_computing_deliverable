    ...
    float* __restrict__ result_vector) 
{
    ...
    // Initialize to safe defaults for threads out of bounds
    int my_row = -1;
    float my_val = 0.0f;
    ...
    // 1. Segmented Inclusive Warp Scan
    // Accumulate values from threads on our "left" if they belong to the same row
    for (int offset = 1; offset < 32; offset *= 2) {
        // Fetch row and value from the thread 'offset' lanes behind us
    ...
    // 2. Identify the "Tail" Thread
    // Peek at the row index of the thread immediately to our "right"
    int next_row = __shfl_down_sync(0xFFFFFFFF, my_row, 1);
    
    // A thread is the tail of a segment within the warp if:
    ...
    // 3. Write-Back via Atomic
    // Only the tail threads execute the atomicAdd, writing the accumulated warp segment sum
    if (i < nnz && is_tail) {
...
__global__ void csr_spmv_custom_1_kernel(int n_rows,
    ...
    int lane = threadIdx.x % 32; // Thread's index within the warp (0-31)
    ...
    if (warp_id < n_rows) {
        ...
        // This uses the register-to-register shuffle instruction
